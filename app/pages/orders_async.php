...
?>
    <form class="mb-3" class="filter-form" action="orders_async.php" data-target=".filter-results">
        ...
        <div class="row mb-3 justify-content-between">
            <div class="col-2">
                <button type="button" class="btn btn-success btn-submit" id="btn-submit">Buscar</button>
                <button type="button" class="btn btn-secondary btn-clear">Limpiar</button>
            </div>
            ...
            <?php else : ?>
                <?php foreach ($arrOrders as $order) : ?>
                    <tr>
                        ...
                        <td><?= $order['status'] ?></td>
                        <td>
                            <button type="button" class="btn btn-primary btn-load-async" data-action="orders_async.php" data-type="record-orders" data-params="<?php echo toObject(['id' => $order['id']]); ?>" data-target="main">
                                Editar
                            </button>
                        </td>
                    </tr>
...
?>
    ...
    <div class="container">
        <form action="orders_async.php">
            ...
            <div class="row mb-3">
                <div class="col-12">
                    <form action="orders_async.php">
                        ...
                        <input type="hidden" name="prueba" value="<?= $order['id'] ?>">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" value="<?= $order['status'] ?>>
                                    <option value=" <?= $order['status'] ?>" selected class="form-control"></option>
                                <option value="1">activo</option>
                                <option value="2">en proceso</option>
                                <option value="3">finalizado</option>
                                <option value="4">Cancelado</option>
            ...
            <div class="row mb-3">
                <div class="col-3">
                    <button type="button" class="btn btn-success btn-form-async" data-action="list-orders" data-target="main">
                        Guardar
                    </button>
                    <button type="button" class="btn btn-dark btn-load-async" data-action="orders_async.php" data-type="list-orders" data-target="main">
                        Cancelar
                    </button>
                </div>
            </div>
...
if ($GPC['type'] == 'save-order') {
    if (empty($GPC['id'])) {
        ...
        $query = "INSERT INTO orders (origen, destino, salida, retorno, total, fecha, hora, status) VALUES ('{$GPC['origen']}', '{$GPC['destino']}', '{$GPC['salida']}', '{$GPC['retorno']}', '{$GPC['total']}', '{$GPC['fecha']}', '{$GPC['hora']}', '{$GPC['status']}')";
    } else {
        $query = "UPDATE orders SET origen = '{$GPC['origen']}', destino = '{$GPC['destino']}', salida = '{$GPC['salida']}', retorno = '{$GPC['retorno']}', total = '{$GPC['total']}', fecha = '{$GPC['fecha']}', hora = '{$GPC['hora']}', status = '{$GPC['status']}' WHERE id = {$GPC['id']}";
    }
